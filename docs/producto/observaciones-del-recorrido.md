...
| C-3 | **Importar la factura para autocompletar el gasto**, y poder adjuntar el comprobante (factura / recibo / documento respaldatorio). No obligatorio, pero posible. | No existe. Necesita `ObjectStorage` (que llega con la tanda C) y un extractor. Ampliado en C-3.bis. |
| C-3.bis | **Y no todos los comprobantes son facturas.** *(usuario, 2026-08-04)* Entra un **ticket** —el del corralón, el del ferretero— que no es una factura y no tiene la misma información ni el mismo valor fiscal. El módulo de importación tiene que contemplar los dos: **PDF legible** (texto embebido, se extrae directo) y **papel fotografiado / PDF escaneado**, que pide **OCR**. Son dos caminos técnicos distintos y hay que decidir cuál entra primero. | No existe. **Dos derivaciones antes de construir:** qué se puede imputar y cómo se registra un ticket frente a una factura → `contador`; y qué respaldo exige el reclamo de la deuda si el comprobante es un ticket → `legal-ph`. |
...
| C-7 | **Traer los gastos del mes anterior.** El 80 % de los gastos son los mismos doce rubros, con los mismos proveedores, todos los meses. *(`administrador-consorcios`, 2026-08-04: **"es el mayor ahorro de tiempo del oficio"** — más que cualquier mejora de navegación.)* | No existe. |
